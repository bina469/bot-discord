...
async function atualizarPainel() {
  try {
    ...
    const status = telefones.map(t =>
      estadoTelefones[t] ? `🔴 ${t} — ${estadoTelefones[t].nome}` : `🟢 ${t} — Livre`
    ).join('\n');
...
client.once('ready', async () => {
  ...
  try {
    ...
    console.error('Erro ao enviar mensagem inicial do ticket:', err);
  }
});
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    /* ===== MENUS (Sair, Forçar, Transferir) ===== */
    const menus = [
      { id: 'menu_sair', handler: 'sair' },
      { id: 'menu_forcar', handler: 'forcar' },
      { id: 'menu_transferir', handler: 'transferir' }
    ];

    for (const menu of menus) {
      if (interaction.isButton() && interaction.customId === menu.id) {
        try {
          await handleMenu(interaction, menu.handler);
        } catch (err) {
          console.error(`Erro no menu ${menu.id}:`, err);
          interaction.reply({ content: '❌ Falha ao abrir menu, tente novamente.', ephemeral: true });
        }
        return;
      }
    }
    ...
    if (interaction.isButton() && interaction.customId === 'abrir_ticket') {
      if (!interaction.member.roles.cache.has(CARGO_TELEFONISTA_ID)) return interaction.reply({ content: '❌ Apenas telefonistas.', ephemeral: true });
      if (ticketsAbertos.has(interaction.user.id)) return interaction.reply({ content: '⚠️ Você já tem ticket aberto.', ephemeral: true });
...
/* ================= HANDLER DE MENUS ================= */
async function handleMenu(interaction, tipo) {
